...
st.markdown("<h1 style='text-align: center; color: grey;'>ANALYSE THIS!</h1>",
    ...
    if df.empty:
        ...
        with tab1:
            ...
            for columns in categorical_columns:
                ...
                df[columns] = label_encoders[columns].fit_transform(
                    df[columns])

            st.header("Maximum")
            st.write("Maximum values in each columns:",
                     df.max(numeric_only=True))

        with tab2:
            st.header("Minimum")
            st.write("Minimum values in each columns:",
                     df.min(numeric_only=True))

        with tab3:
            st.header("Average")
            st.write("Average values in each columns:",
...
def feature():
    ...
    if df.empty:
        ...
        if d2 == "":
            ...
            st.info(
                'The highest value column has higher dependencies in the feature variable', icon="ℹ️")
